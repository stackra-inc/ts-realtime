...
var tsHttp = require('@stackra/ts-http');
...
exports.RealtimeManager = class _RealtimeManager extends tsSupport.MultipleInstanceManager {
  static {
  ...
  */
  logger = new tsLogger.Logger(_RealtimeManager.name);
  // ─────────────────────────────────────────────────────────────────────────
  // Lifecycle hooks
  // ─────────────────────────────────────────────────────────────────────────
  /**
  ...
  */
  async createDriverAsync(_driver, config) {
    ...
    this.emit(contracts.RealtimeEvents.CONNECTED, {
  ...
  */
  async disconnect(name) {
    ...
    if (this.hasInstance(connectionName)) {
      ...
      this.emit(contracts.RealtimeEvents.DISCONNECTED, {
        connection: connectionName,
        code: 1e3
      });
    }
...
exports.RealtimeManager = _ts_decorate([
  tsContainer.Injectable(),
  _ts_param(0, tsContainer.Inject(contracts.REALTIME_CONFIG)),
  _ts_param(1, tsContainer.Inject(contracts.REALTIME_CONNECTOR)),
  _ts_param(2, tsContainer.Optional()),
  _ts_param(2, tsContainer.Inject(contracts.EVENT_EMITTER_MANAGER)),
  _ts_param(3, tsContainer.Optional()),
  _ts_param(3, tsContainer.Inject(contracts.TAB_COORDINATOR)),
...
var EchoConnection = class {
  ...
  status = contracts.ConnectionStatus.Disconnected;
  ...
  */
  isConnected() {
    return this.status === contracts.ConnectionStatus.Connected;
  }
  /**
  ...
  */
  connect() {
    ...
    this._setStatus(contracts.ConnectionStatus.Connecting);
    const pusherInstance = new Pusher__default.default(this.config.key, {
    ...
    this.echo = new Echo__default.default(echoOptions);
  ...
  */
  disconnect() {
    ...
    this._setStatus(contracts.ConnectionStatus.Disconnected);
  }
  // ---------------------------------------------------------------------------
  ...
  */
  _assertConnected() {
    if (!this.echo || this.status !== contracts.ConnectionStatus.Connected) {
  ...
  _bindPusherEvents(pusher) {
    pusher.connection.bind("state_change", (states) => {
      switch (states.current) {
        case "connected":
          this._reconnectAttempts = 0;
          this._setStatus(contracts.ConnectionStatus.Connected);
          this._resubscribeChannels();
          break;
        case "connecting":
        case "unavailable":
          if (this.status !== contracts.ConnectionStatus.Connecting && this.status !== contracts.ConnectionStatus.Reconnecting) {
            this._setStatus(contracts.ConnectionStatus.Connecting);
          }
          break;
        case "disconnected":
          if (this.status === contracts.ConnectionStatus.Disconnected) return;
  ...
  */
  _startReconnect() {
    if (this.status === contracts.ConnectionStatus.Disconnected) return;
    this._setStatus(contracts.ConnectionStatus.Reconnecting);
  ...
  */
  _resubscribeChannels() {
    ...
    for (const [name, wrapper] of this.channels.entries()) {
      ...
      if (tsSupport.Str.startsWith(name, "private:")) {
...
exports.LaravelEchoConnector = class _LaravelEchoConnector {
  static {
  ...
  */
  logger = new tsLogger.Logger(_LaravelEchoConnector.name);
  ...
  */
  createNoOpConnection() {
    ...
    return {
      ...
      getStatus: /* @__PURE__ */ __name(() => contracts.ConnectionStatus.Disconnected, "getStatus"),
...
exports.LaravelEchoConnector = _ts_decorate2([
  tsContainer.Injectable()
], exports.LaravelEchoConnector);
...
exports.RealtimeEventsBridgeListener = class _RealtimeEventsBridgeListener {
  static {
  ...
  */
  logger = new tsLogger.Logger(_RealtimeEventsBridgeListener.name);
  /**
...
exports.RealtimeEventsBridgeListener = _ts_decorate3([
  tsContainer.Injectable(),
  _ts_param2(0, tsContainer.Inject(contracts.REALTIME_CONNECTOR)),
  _ts_param2(1, tsContainer.Optional()),
  _ts_param2(1, tsContainer.Inject(contracts.EVENT_EMITTER_MANAGER)),
  _ts_metadata2("design:type", Function),
  _ts_metadata2("design:paramtypes", [
    typeof LaravelEchoConnector === "undefined" ? Object : LaravelEchoConnector,
    typeof EventEmitterManagerLike === "undefined" ? Object : EventEmitterManagerLike
  ])
], exports.RealtimeEventsBridgeListener);
...
exports.SocketIdMiddleware = class _SocketIdMiddleware {
  static {
  ...
  */
  logger = new tsLogger.Logger(_SocketIdMiddleware.name);
  /**
...
exports.SocketIdMiddleware = _ts_decorate4([
  tsHttp.HttpMiddleware({
    priority: 25,
    name: "realtime:socket-id"
  }),
  _ts_param3(0, tsContainer.Inject(contracts.REALTIME_MANAGER)),
  _ts_metadata3("design:type", Function),
  _ts_metadata3("design:paramtypes", [
    typeof RealtimeManager === "undefined" ? Object : RealtimeManager
  ])
], exports.SocketIdMiddleware);
...
var InjectRealtime = /* @__PURE__ */ __name((connectionName) => tsContainer.Inject(getRealtimeConnectionToken(connectionName)), "InjectRealtime");
...
exports.RealtimeModule = class _RealtimeModule {
  static {
  ...
  */
  static logger = new tsLogger.Logger(_RealtimeModule.name);
  /**
  ...
  */
  static hasCredentials() {
    try {
      const pusherKey = String(tsSupport.Env.get("VITE_PUSHER_APP_KEY", "") ?? "").trim();
      const pusherHost = String(tsSupport.Env.get("VITE_PUSHER_HOST", "") ?? "").trim();
      const reverbKey = String(tsSupport.Env.get("VITE_REVERB_APP_KEY", "") ?? "").trim();
      const reverbHost = String(tsSupport.Env.get("VITE_REVERB_HOST", "") ?? "").trim();
  ...
  */
  static forRoot(config) {
    ...
    return {
      ...
      providers: [
        {
          provide: contracts.REALTIME_CONFIG,
          useValue: config
        },
        {
          provide: exports.RealtimeManager,
          useClass: exports.RealtimeManager
        },
        {
          provide: contracts.REALTIME_MANAGER,
          useExisting: exports.RealtimeManager
        },
        {
          provide: contracts.REALTIME_CONNECTOR,
          useClass: exports.LaravelEchoConnector
        },
      ...
      exports: [
        exports.RealtimeManager,
        contracts.REALTIME_MANAGER,
        ...connectionTokens
      ]
    };
  }
};
exports.RealtimeModule = _ts_decorate5([
  tsContainer.Module({})
...
var IRedisConnection = class _IRedisConnection {
  ...
  */
  logger = new tsLogger.Logger(_IRedisConnection.name);
  ...
  */
  connect() {
    if (this.status === contracts.ConnectionStatus.Connected) {
      return;
    }
    this._setStatus(contracts.ConnectionStatus.Connecting);
    this._reconnectAttempts = 0;
...
exports.RedisConnector = class IRedisConnector {
  static {
...
exports.RedisConnector = _ts_decorate6([
  tsContainer.Injectable(),
  _ts_param4(0, tsContainer.Inject(contracts.REDIS_MANAGER)),
  _ts_metadata4("design:type", Function),
  _ts_metadata4("design:paramtypes", [
    typeof RedisManager === "undefined" ? Object : RedisManager
  ])
], exports.RedisConnector);
var realtime = tsContainer.inject(contracts.REALTIME_MANAGER);
var channelRefCounts = /* @__PURE__ */ new Map();
function useChannel(channelName, eventName, options) {
  const manager = react.useInject(contracts.REALTIME_MANAGER);
  if (!manager) {
  ...
  const connRef = react$1.useRef(null);
  const channelRef = react$1.useRef(null);
  react$1.useEffect(() => {
    if (!enabled) return;
    let cancelled = false;
    let unsubscribe;
...
function usePresence(channelName) {
  ...
  const [members, setMembers] = react$1.useState([]);
  const [error, setError] = react$1.useState(null);
  const [connected, setConnected] = react$1.useState(false);
  const connRef = react$1.useRef(null);
  const channelRef = react$1.useRef(null);
...
function useRealtime() {
  ...
  const [status, setStatus] = react$1.useState(contracts.ConnectionStatus.Disconnected);
  const connRef = react$1.useRef(null);
  react$1.useEffect(() => {
    let cancelled = false;
    let unsubscribe;
    manager.connection().then((conn) => {
      ...
      if (!cancelled) {
        setStatus(contracts.ConnectionStatus.Error);
      }
    });
    return () => {
      cancelled = true;
  ...
  return {
    status,
    isConnected: status === contracts.ConnectionStatus.Connected,
    manager
  };
}
__name(useRealtime, "useRealtime");
var InjectRealtimeManager = /* @__PURE__ */ __name(() => tsContainer.Inject(contracts.REALTIME_MANAGER), "InjectRealtimeManager");
...
exports.EchoConnection = EchoConnection;
exports.InjectRealtime = InjectRealtime;
exports.InjectRealtimeManager = InjectRealtimeManager;
exports.PresenceChannelWrapper = PresenceChannelWrapper;
exports.RealtimeChannelError = RealtimeChannelError;
exports.RealtimeConnectionError = RealtimeConnectionError;
exports.RealtimeError = RealtimeError;
exports.RedisConnection = IRedisConnection;
exports.defineConfig = defineConfig;
exports.getRealtimeConnectionToken = getRealtimeConnectionToken;
